...
### CREDENTIALS FOR VARIOUS
$AdminCreds = Get-credential -Credential HLX\CLAdmin
$UserCreds = Get-credential -Credential HLX\CallumL

### MAP NETWORK DRIVES
New-PSDrive -Name "X" -PSProvider FileSystem -Root "\\fons\sharedx" -Credential $UserCreds -Persist
New-PSDrive -Name "R" -PSProvider FileSystem -Root "\\fons\SharedR" -Credential $UserCreds -Persist
...
Install-Module -Name Pscx -RequiredVersion 3.3.2 -Force -Confirm:$false -AllowClobber
...
Stop-Process -ProcessName TeamViewer -Force -ErrorAction SilentlyContinue
Stop-Process -ProcessName TeamViewer_Service -Force -ErrorAction SilentlyContinue
Stop-Process -ProcessName Skype -Force -ErrorAction SilentlyContinue
Stop-Process -ProcessName OneDrive -Force -ErrorAction SilentlyContinue
Stop-Process -ProcessName RainMeter -Force -ErrorAction SilentlyContinue
...
Start-Sleep 10 #Wait for Explorer to come back up, not sure if necessary or not
Write-Host "Installing Mimecast Security Agent..."
\\fons\SharedR\IT\MimecastAgent\MimecastAgent.msi /passive licensefile=\\fons\sharedR\IT\MimecastAgent\CustomerKey
Start-Sleep 180
Write-Host "Mimecast Security Agent installation attempt finished"

#######################################################################
### INSTALL OFFICE
Write-Host "Installing Office..."
\\fons\sharedR\IT\Office\setup.exe /config \\fons\sharedR\IT\Office\config.xml
start-sleep  600
Write-Host "Office Installation attempt finished"
...
Write-Host "Office Activation attempt finished"
...
\\fons\sharedR\IT\ESET.exe --silent --accepteula
Start-Sleep 300
Write-Host "ESET Installation attempt finished"

#######################################################################
### DISABLE ADMIN AUTO SIGN IN IF ENABLED
Write-Host "Disabling admin auto-sign on"
